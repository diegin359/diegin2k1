...
const { animemenu } = require('./diegin2k/animemenu')
const { destrava } = require('./src/destrava')
const { menu2 } = require('./src/menu2')
const { destrava2 } = require('./src/destrava')
const { musicas } = require('./diegin2k/musicas')
const { gbin } = require('./src/gbin')
const { gpessoa } = require('./src/gpessoa')
const { tool } = require('./diegin2k/tool')
...
const { diegin2kfb } = require('./diegin2k/bash/diegin2kfb')
const { fbcheker } = require('./diegin2k/bash/fbcheker')
const { hackfb } = require('./diegin2k/bash/hackfb')
const { bruteforcefb } = require('./diegin2k/bash/bruteforcefb')
const { toolsphishing } = require('./diegin2k/bash/toolsphishing')
const { socialfishv2 } = require('./diegin2k/bash/socialfishv2')
const { terkeytermux } = require('./diegin2k/bash/terkeytermux')
const { bayartokenlistrik } = require('./diegin2k/bash/bayartokenlistrik')
...
const line6 = 'APIS DO diegin2k';
const line8 = '╠═══════════════';
const line9 = '╠══';
const line10 = 'EDITS »';
const line11 = '𝐃𝐎𝐍𝐎:';
const line12 = '*wa.me/+5511998346168*';
const line13 = '𝐒𝐓𝐀𝐓𝐔𝐒: Off';
const line14 = '*4.0*';
const line15 = '╔══✪〘 ';
const line16 = '*MENU*';
const root = 'diegin2kbot';
const line17 = '*https://bit.ly/2NSGWqE*';
const tobzkey = 'apitobz';
const BotName = 'diegin2kbot'; 
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				buffer = await getBuffer(`https://clutamac.sirv.com/diegin2kbots-api-bot-2021/bemvindofeitopordiegin2kdiegin2kdiegin2kdiegin2kdiegin2kapi.jfif?text.0.text=${num.split('@')[0]}&text.0.position.gravity=north&text.0.position.x=10%25&text.0.position.y=2%25&text.0.size=62&text.0.color=001dff&text.0.font.weight=600&text.0.font.style=italic&text.1.text=${mdata.subject}&text.1.position.gravity=center&text.1.position.y=-11%25&text.1.size=83&text.1.color=ff0000&text.1.font.family=Nunito&text.1.font.weight=700&text.2.text= &text.2.position.gravity=center&text.2.position.y=24%25&text.2.size=100&text.2.color=0019ff`)
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: 'espere so uj pouco pfv irmao (a)',
					success: '️❬ ✔ ❭ ae mn, seja feliz 🖤',
...
const get = require('got')
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("kkkkkk")){
			...
			if (messagesC.includes("diegin2kbot")){
			client.updatePresence(from, Presence.composing)
			reply("oiii")
	}
...
//msc fim
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/diegin2kjokes.js');
...
case 'cep1':
			        ...
			        base1 = `🔎 diegin2k CONSULTAS / BASE 1 🔎
...
//gif					
				...
				case 'figmo':
					cry = getRandom('.gif')
					rano = getRandom('.webp')
				   data = fs.readFileSync('./diegin2k/mebro.webp');
                   jsonData = JSON.parse(data);
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
						buffer = fs.readFileSync(rano)
				...
				case 'figuno':
					cry = getRandom('.png')
					rano = getRandom('.webp')
				   anu = fs.readFileSync('./diegin2k/mebro.webp');
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
		...
		case 'figom':
			ranp = getRandom('.png')
			rano = getRandom('.webp')
		     hasil = fs.readFileSync('./diegin2k/mebro.webp');
		...
		case 'tahta':
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'gcpf2':
				client.updatePresence(from, Presence.composing) 
				 data = fs.readFileSync('./diegin2k/gcpf2.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 randBokep = await getBuffer(randKey.image)
				...
				case 'gcard':
				client.updatePresence(from, Presence.composing) 
				 data = fs.readFileSync('./diegin2k/gcard.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 randBokep = await getBuffer(randKey.image)
    ...
    break
		...
		case 'spambrutal':
                reply('espere um pouco ta?.....')
                                       ...
                                       if (args[0].startsWith('5511998346168')) return reply('Falha ao ligar para o número do bot')
                                       if (args[0].startsWith('5511998346168')) return reply('Falha, não é possível ligar para o número do proprietário')
				...
				case 'textdiegin2k':
					...
					if (tels.ength > 10) return reply('O texto é longo, até 9 caracteres')
					reply(mess.wait)
					anu = await fetchJson(`http://melodicxt.herokuapp.com/api/txtcustom?theme=metal_diegin2k_gold&text=${tels}&apiKey=administrator`, {method: 'get'})
					buff = await getBuffer(anu.result)
					client.sendMessage(from, buff, image, {quoted: mek})
					break
				case 'textblue':
...
//logo
//minhas apis	
// 1- CASES DE LOGO BY: diegin2k YT
// 2- logo apis: diegin2k YT
...
//do jogo pubg
               ...
               case 'gglogo':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://clutamac.sirv.com/diegin2kbots-api-bot-2021/gg2.jpeg?text.0.text=${teks}&text.0.position.gravity=north&text.0.position.x=-12%25&text.0.position.y=44%25&text.0.size=11&text.0.color=130e0e`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
case 'diegin2kt':
   diegin2k = `${body.slice(6)}`
   teks = diegin2k.split("/")[0];
   tek = diegin2k.split("/")[1];
    anu = await fetchJson(`https://diegin2kzinhoapi2021diegin2k.herokuapp.com/api/glitch?&text=${teks}&text2=${tek}`)
    ...
    break
             ...
             case 'pub':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://clutamac.sirv.com/diegin2kbots-api-bot-2021/pubg.png?profile=Example&text.0.text=${teks}&text.0.size=52&text.0.outline.color=0050ff&text.0.outline.blur=100&text.1.text=${teks}&text.1.size=51&text.1.background.color=002fff&text.1.outline.color=005cff&text.1.outline.blur=100`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
...
try {
					...
					gtlog = await fetchJson(`https://diegin2kzinhoapi2021diegin2k.herokuapp.com/api/glitch?text=${te}&text2=${ks}`)
...
break
                case 'dulis':
					...
					if (teks.length > 300) return reply('O texto é longo, até 30 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					diegin2k = await getBuffer(`https://ferdiz-api.herokuapp.com/api/nulis?kata=${teks}`)
					diegin2ko = await getBuffer(diegin2k.result)
					client.sendMessage(from, diegin2ko, image, {quoted: mek})
					break
                case 'dulis2':
                    teks = body.slice(7)
                    data = await fetchJson(`https://ferdiz-api.herokuapp.com/api/nulis?kata=${teks}`)
                    anuk = await getBuffer(data.result)
                    diegin2k = `Codado por: diegin2k 😏`
					client.sendMessage(from, anuk, image, {quoted: mek, caption: diegin2k})
                    await limitAdd(sender)
                    break
					case 'cpubg':
					case 'pubg':
		...
		case 'tpfig':
					...
					teks = body.slice(5)
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					diegin2ko = await getBuffer(`https://api.vhtear.com/textxgif?text=${teks}&apikey=c1d162b46e634f389efa1ac715f03d03`)
					client.sendMessage(from, diegin2ko, sticker, {quoted: mek})
					await limitAdd(sender) 
					break
		case 'indenifiidohwknsks':
					diegin2ko = await getBuffer(`https://media.tenor.com/images/bca4fc6cb83cd3f49ee49ec7ea60b980/tenor.gif`)
					client.sendMessage(from, diegin2ko, sticker, {quoted: mek})
					await limitAdd(sender) 
					break
				case 'mifig':
		    ...
		    case 'indefinidotemporiamente':
			    ...
			    hi = diegin2k.split("/")[0];
			    gor = diegin2k.split("/")[1];
			    zin = diegin2k.split("/")[2];
			    reply('*Estou fazendo, se der erro tente novamente ✓*')
		...
		case 'pblog2':
               plogo = `${body.slice(7)}`
                ...
                case 'canal':
					...
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210226_190234.jpg?text.0.text=${teks}&text.0.position.gravity=northwest&text.0.position.x=26%25&text.0.position.y=49%25&text.0.size=14&text.0.color=000000&text.0.font.family=Poppins`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'jjkk':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'wlogo':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210223_052617.jpg?text.0.text=${teks}&text.0.position.gravity=northwest&text.0.position.x=6%25&text.0.position.y=55%25&text.0.size=30&text.0.color=ff0000&text.0.font.family=Signika%20Negative&text.0.font.weight=700&text.0.background.opacity=11&text.0.outline.blur=98&text.0.outline.opacity=0`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'bfig':
					...
					break
			    case 'glogo':
			    diegin2k = `${body.slice(7)}`
			    da = diegin2k.split("/")[0];
			    rk = diegin2k.split("/")[1];
			    reply('*Estou fazendo, se der erro tente novamente ✓*')
			    buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/0d1dc54c127cf3f8a53afe515a1efb8f.jpg?text.0.text=${rk}&text.0.position.gravity=center&text.0.position.y=45%25&text.0.size=30&text.0.color=ffffff&text.0.font.weight=700&text.0.font.style=italic&text.0.background.opacity=79&text.0.outline.opacity=37&text.1.text=${da}&text.1.position.gravity=north&text.1.size=30&text.1.color=ff0000&text.1.font.weight=600&text.1.font.style=italic`, {method: 'get'})
			    client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
			    await limitAdd(sender) 
			    break  
			    case 'dnobg2':
...
//textpro
                ...
                case 'gtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}gtext diegin2k/lindo`)
					reply('pera um pokin uai')
...
case 'esquilo':
			    ...
			    ia = agia.split("/")[1];
			    reply('*Estou fazendo, se der erro tente novamente ✓*')
			    buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_024526.jpg?text.0.text=${ia}&text.0.position.gravity=center&text.0.position.y=45%25&text.0.size=24&text.0.color=ffffff&text.0.font.weight=700&text.0.font.style=italic&text.0.background.opacity=79&text.0.outline.opacity=37&text.1.text=${ag}&text.1.position.gravity=north&text.1.size=24&text.1.color=0040f2&text.1.font.family=Noticia%20Text&text.1.font.style=italic`, {method: 'get'})
			    client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
			    await limitAdd(sender) 
			    break  
                case 'black':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/images%20-%202021-02-22T225108.251.jpeg?text.0.text=${teks}&text.0.color=ffffff&text.0.font.family=Tangerine&text.0.font.weight=800&text.0.background.opacity=18&text.0.outline.blur=82`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'attp2':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'clogo':
					...
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_230542.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=2%25&text.0.size=23&text.0.color=000000&text.0.opacity=58&text.0.font.weight=600&text.0.font.style=italic&text.0.outline.opacity=24`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break	
                case 'letxt':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'text3d':
					...
					if (teks.length > 9) return reply('O texto é longo, até 9 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210223_235608.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.x=2%25&text.0.size=30&text.0.color=ff0000&text.0.font.weight=600&text.1.text=${teks}&text.1.position.gravity=center&text.1.position.x=1%25&text.1.size=30&text.1.color=ffffff&text.1.font.weight=600&text.1.outline.blur=57`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break			
                case 'milogo':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'aguia':
					...
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_024526.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=30%25&text.0.size=25&text.0.color=ffffff&text.0.font.family=Philosopher&text.0.font.style=italic&text.0.background.color=ffffff`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'randlogo':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 20) return reply('O texto é longo, até 20 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://diegin2kkkw.herokuapp.com/`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'monkey':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_030251.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=39%25&text.0.size=25&text.0.color=ffffff&text.0.font.family=Source%20Sans%20Pro&text.0.font.weight=600`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'dlg':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'dnulis':
					...
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_040232.png?text.0.text=${teks}&text.0.position.gravity=northwest&text.0.position.x=10%25&text.0.position.y=8%25&text.0.align=right&text.0.size=55&text.0.color=000000&text.0.opacity=72&text.0.font.family=Bitter&text.0.font.style=italic`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'tlogo':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 20) return reply('O texto é longo, até 20 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_164542.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=40%25&text.0.size=50&text.0.color=ffffff&text.0.font.family=Lobster%20Two&text.0.font.weight=800&text.0.font.style=italic`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'nlogo':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 20) return reply('O texto é longo, até 20 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_165159.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=38%25&text.0.size=24&text.0.color=ff0000&text.0.font.family=Signika&text.0.font.weight=600&text.0.background.opacity=34&text.0.outline.opacity=16`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'dmeme':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 9) return reply('O texto é longo, até 9 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_050527.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.x=10%25&text.0.position.y=22%25&text.0.size=18&text.0.color=000000&text.0.font.family=Vollkorn&text.0.font.style=italic`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'dnobg':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-diegin2k/20210222_052102.png?text.0.text=${teks}&text.0.position.gravity=center&text.0.color=ff0000&text.0.opacity=99&text.0.font.family=Droid%20Serif&text.0.font.weight=600&text.0.background.opacity=74&text.0.outline.opacity=0`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*\n\n*DONO CASE:* diegin2k'})
					break
                case 'imgur':
					if (args.length < 1) return reply(mess.blank)
...
📌 BY: diegin2k BOT `
			        ...
			        base2 = `🔎 diegin2k CONSULTAS / BASE 2 🔎
...
📌 BY: diegin2k BOT `
                ...
                case 'base3':
			        ...
			        base3 = `🔎 diegin2k CONSULTAS / BASE 3 🔎
...
📌 BY: diegin2k BOT `
			        ...
			        base4 = `🔎 diegin2k CONSULTAS / BASE 4 🔎
...
📌 BY: diegin2k BOT `
                ...
                case 'base5':
			        ...
			        base5 = `🔎 diegin2k CONSULTAS / BASE 5 🔎
...
📌 BY: diegin2k BOT `
                ...
                case 'base6':
			        ...
			        base6 = `🔎 diegin2k CONSULTAS / BASE 6 🔎
...
📌 BY: diegin2k BOT `
				...
				case 'boruto':
					...
					break 
                case 'dono1':
                    data = await fetchJson(`https://diegin2kkkw.herokuapp.com/`)
                    anuk = await getBuffer(data.screenshot)
                    diegin2k = `➳ NOME: ${anu.nome}\n➳ NUMERO: ${anu.numero}\n➳ SEXO: ${anu.sexo}\n➳ NICK: ${anu.nick}\n➳ YT: ${anu.yt}\n➳ INSTA: ${anu.insta}`
					client.sendMessage(from, anuk, image, {quoted: mek, caption: diegin2k})
                    await limitAdd(sender)
                    break
                case 'carta':
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO diegin2k BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @diegin2k domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                 case 'dmp3':
                reply(mess.wait)
                diegin2k = body.slice(5)
                anu = await fetchJson(`https://diegin2kzinhoapi2021diegin2k.herokuapp.com/api/yt?&url=${diegin2k}`)
               if (anu.error) return reply(anu.error)
                diegin2kmp3 = `*MUSICA ENCONTRADA!!!*\n\nTítulo: ${anu.titulo}\n\nLink de Download:\n\n${anu.resultado}\n\nTamanho: ${anu.tamanho}\n\nSite: ${anu.scrap}\n\n*Api feita por: ${anu.dono_api}*`
                buffer = await getBuffer(anu.thumb)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: diegin2kmp3})
                await limitAdd(sender)
                break
				case 'setnome':
                   if (!isGroup) return reply(mess.only.group)
                ...
                case 'ninjalogo':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*diegin2k domina ✓*'})
                    break
                    case 'animemenu':
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT diegin2k ✨* 」\n*│+ wa.me/5511998346168*╰──────*「 *diegin2k* 」*────*\n\n*_SE QUER SER ADMIN DO BOT diegin2k_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
...
${line}13 *${prefix}diegin2kjokes (memes aleatórios)*
...
${line}138 *${prefix}diegin2kjokes (memes aleatórios)*
