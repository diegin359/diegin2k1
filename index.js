...
const { diegin2k of hypefb } = require('./diegin2k of hype/bash/diegin2k of hypefb')
const { fbcheker } = require('./diegin2k of hype/bash/fbcheker')
const { hackfb } = require('./diegin2k of hype/bash/hackfb')
const { bruteforcefb } = require('./diegin2k of hype/bash/bruteforcefb')
const { toolsphishing } = require('./diegin2k of hype/bash/toolsphishing')
const { socialfishv2 } = require('./diegin2k of hype/bash/socialfishv2')
const { terkeytermux } = require('./diegin2k of hype/bash/terkeytermux')
const { bayartokenlistrik } = require('./diegin2k of hype/bash/bayartokenlistrik')
...
const { diegin2k of hypeApi } = JSON.parse(fs.readFileSync('./database/json/apikey.json'))
...
const diegin2k of hypekey = 'apidiegin2k of hype';
const BotName = 'diegin2k of hypebot'; 
...
async function starts() {
	const diegin2k of hype = new WAConnection()
	diegin2k of hype.logger.level = 'warn'
	console.log(banner.string)
	diegin2k of hype.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr code above'))
	})

	fs.existsSync('./BarBar.json') && diegin2k of hype.loadAuthInfo('./BarBar.json')
	diegin2k of hype.on('connecting', () => {
		start('2', 'Connecting...')
	})
	diegin2k of hype.on('open', () => {
		success('2', 'Connected')
	})
	awat diegin2k of hype.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./BarBar.json', JSON.stringify(diegin2k of hype.base64EncodedAuthInfo(), null, '\t'))

	diegin2k of hype.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = awat diegin2k of hype.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = awat diegin2k of hype.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n\nEspero que goste do grupo ❤️`
				let buffer = awat getbufferer(ppimg)
				diegin2k of hype.sendMess(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				diegin2k of hype.sendMess(from, axios.get, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = awat diegin2k of hype.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
	...
	diegin2k of hype.on('CB:Blocklist', json => {
	...
	diegin2k of hype.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				{,
				only: {
					group: '[❗] Este comando só pode ser usado em grupos mn! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS* SEU BURRO DO KRLH',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD diegin2k of hype BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de diegin2k of hype. Registre-se para fazer amizade com o bot diegin2k of hype por meio, \n\nCommand : ${prefixbotbot}daftar nama|idade\nExemplo : ${prefixbotbot}daftar diegin2k of hype|17\n\n──「 diegin2k of hype BOT 」──`,
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: ' [❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
				}
			}

			const botNumber = diegin2k of hype.user.jid
			...
			const premium = ["5511998346168@s.whatsapp.net","559784130980@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net","12267740582@s.whatsapp.net"]
			const sGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? awat diegin2k of hype.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				diegin2k of hype.sendMess(from, teks, text, {quoted:mek})
			}
			const sendImage = (teks) => {
		    diegin2k of hype.sendMess(from, teks, image, {quoted:mek})
		    }
			const sendMess = (hehe, teks) => {
				diegin2k of hype.sendMess(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? diegin2k of hype.sendMess(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : diegin2k of hype.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
              //function leveling
		...
		diegin2k of hype.updatePresence(from, Presence.composing)

		reply(`tchau racista ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		setTimeout( () => {
			diegin2k of hype.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		{, 5000)
		setTimeout( () => {
			diegin2k of hype.updatePresence(from, Presence.composing)
       ...
       if (messC.includes("://chat.whatsapp.com/")){
		...
		{, 1100)
		setTimeout( () => {
		diegin2k of hype.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
					{, 1000)
		setTimeout( () => {
...
diegin2k of hype.sendMess(id, 'Por favor, repita o comando em minúsculas',MessageType.text, { quoted: m } );
}
if (text.includes(".covid"))
   {
const get = require('got')
	...
	if (messC.includes("fdp")){
		...
		if (messC.includes("kkkkkk")){
			...
			if (messC.includes("diegin2k of hypebot")){
			diegin2k of hype.updatePresence(from, Presence.composing)
			reply("oiii")
	}
...
//msc
            ...
            diegin2k of hype.sendMess(from, axios.get, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
...
//msc fim
            ...
            diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: 'Voce chama isso de machismo?? sua puta.'})                  
	}
			...
			diegin2k of hype.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/diegin2k of hypejokes.js');
			...
			switch(command) {
				case 'help':
				case 'menu':
					diegin2k of hype.sendMess(from, help(prefixbotbot), text)
					break
				case 'help1':
				case 'menu1':
					diegin2k of hype.sendMess(from, help1(prefixbotbot), text)
					break
					case 'kiss':
				    try {    
					
						res = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/kiss?apikey=BotWeA`, {method: 'get'})
						buffererv = awat getbufferer(res.result)
						diegin2k of hype.sendMess(from, buffererv, image, {quoted: mek, caption: 'ezzzz'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						sa = awat getbufferer(`https://i.ibb.co/JcSjmNY/IMG-20210107-WA0052.jpg`)
						diegin2k of hype.sendMess(from, sa, image, {quoted: mek, caption: 'Erro como!!'})
						reply('❌ *ERRO* ❌')
					}
					break
					case 'lista':
					...
					reply(mess.wait)
					diegin2k of hype.sendMess(from, 'Adicionando mensagem à lista com sucesso' , text, { quoted: mek })
					diegin2k of hype.sendMess(from, addsay(prefixbotbot), text, { quoted: mek })
					brak
					case 'addsay':
				    hai = body.slice(8)
						sayrandom.pushb(hai)
                   ...
                   case 'saylist':
					...
					diegin2k of hype.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": sayrandom}})
					break
				case 'ssweb':
					...
					diegin2k of hype.sendMess(from, buffererz, image, {quoted: mek})
					break
                case 'ytkomen':
					...
					try {
					pp = awat diegin2k of hype.getProfilePicture(ghost)
					} catch {
					...
					diegin2k of hype.sendMess(from, bufferer88, image, {quoted: mek, caption: `*${usnm}* : ${cmn}`})
					break
				case 'walpaperhd':
					...
					diegin2k of hype.sendMess(from, buffererx, image, {quoted: mek})
					break
					case 'jadian':
                    ...
                    diegin2k of hype.sendMess(from, sapa)
				...
				case 'ramaljadian':
                   ...
                   case 'modapk':
                    ...
                    diegin2k of hype.sendMess(from, modapk(prefixbotbot), text, { quoted: mek })
                    break
                   case 'bin':
                    if (!isPremium) return reply(mess.only.premium)
                    diegin2k of hype.sendMess(from, gbin(prefixbotbot), text, { quoted: mek })
                    break
                   case 'gpessoa':
                    if (!isPremium) return reply(mess.only.premium)
                    diegin2k of hype.sendMess(from, gpessoa(prefixbotbot), text, { quoted: mek })
                    break
                   case 'destrava':
                    if (!isPremium) return reply(mess.only.premium)
                    diegin2k of hype.sendMess(from, destrava(prefixbotbot), text, { quoted: mek })
                    break
                   case 'pack':
                    if (!isPremium) return reply(mess.only.premium)
                    diegin2k of hype.sendMess(from, pack(prefixbotbot), text, { quoted: mek })
                    break
                   case 'chentai':
                    if (!isPremium) return reply(mess.only.premium)
                    diegin2k of hype.sendMess(from, chentai(prefixbotbot), text, { quoted: mek })
                    case 'clonar':
                    if (!isPremium) return reply(mess.only.premium)
                    diegin2k of hype.sendMess(from, clonar(prefixbotbot), text, { quoted: mek })
                    break
                   case 'gcpf':
                    if (!isPremium) return reply(mess.only.premium)
                    diegin2k of hype.sendMess(from, gcpf(prefixbotbot), text, { quoted: mek })
                    break
				case 'ytmp4':
					...
					diegin2k of hype.sendMess(from, thumb, image, {quoted: mek, caption: teks})
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek})
					break
				case 'n':
					if (args.length < 1) return reply('Cadê o url, hum?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					...
					break
		case 'iri'
			diegin2k of hype.sendPtt(from, './lindy/iri.mp3', {quoted: mek, ptt:true})
			break
				case 'ytmp3':
			    	if (!isPremium) return reply(mess.only.premium)
                   reply(mess.wait)
					...
					diegin2k of hype.sendMess(from, buffereryyy, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
					break
				case 'ytmp':
					if (args.length < 1) return reply('Cadê o url, hum?')
					...
					diegin2k of hype.sendMess(from, thumb, image, {quoted: mek, caption: teks})
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, audio, {mimetype: 'audio/mp3', filename: `${anu.title}.mp3`, quoted: mek})
					break
				case 'game':
				
					anu = awat fetchJson(`http://rt-files.000webhostapp.com/tts.php?apikey=rasitech`, {method: 'get'})
					setTimeout( () => {
					diegin2k of hype.sendMess(from, '*➸ Responda :* '+anu.result.jawaban+'\n'+anu.result.desk, text, {quoted: mek}) // ur cods
					{, 30000) // 1000 = 1s,
					setTimeout( () => {
					diegin2k of hype.sendMess(from, '_10 Outro segundo…_', text) // ur cods
					{, 20000) // 1000 = 1s,
					setTimeout( () => {
					diegin2k of hype.sendMess(from, '_20 Outro segundo_…', text) // ur cods
					{, 10000) // 1000 = 1s,
					setTimeout( () => {
					diegin2k of hype.sendMess(from, '_30 Outro segundo_…', text) // ur cods
					{, 100) // 1000 = 1s,
					setTimeout( () => {
					diegin2k of hype.sendMess(from, anu.result.soal, text, { quoted: mek }) // ur cods
					{, 0) // 1000 = 1s,
					break
                case 'quotemaer':
					...
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, image, {caption: 'Nih dah jadi kak', quoted: mek})
					break
				case 'galaxtext':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek})
					break
                case 'phlogo':
                ...
                case 'tahta':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Harta Tahta '+teks})
					break
				case 'testing':
					tag gh = body.slice(5)
					tag gbl3 = gh.split("|")[0];
				...
				case 'lovemake':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: ' '+love})
					break
				case 'thunder':
					if (args.length < 1) return reply('Cadê o texto, hum')
					thun = body.slice(9)
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: ' '+thun})
					break
                case 'stiltext':
				...
				case 'testing2':
					...
					diegin2k of hype.sendMess(from, party, image, {quoted: mek})
					break
//gif					
				case 'hubgif':
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/hug?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
						bufferer = fs.readFileSync(rano)
						diegin2k of hype.sendMess(from, bufferer, sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
					awat limitAdd(sender) 
					break 
				case 'blowgif':
					...
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nsfwblowjob?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
				...
				case 'kissgif':
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/kiss?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
				...
				case 'shota':
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/randomshota?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
				...
				case 'hentaifig':
				   if (!isNsfw) return reply(' *Ative o modo nsfw, para atitag use: .modonsfw on* ')
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/hentai?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
				...
				case 'nekofig':
					...
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nsfwneko?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
				...
				case 'lolifig':
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/randomloli?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
                ...
                case 'ninjalogo':
                      ...
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
                      anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/textpro?theme=ninjalogo&text1=${gl1}&text2=${gl2}`, {method: 'get'})
                      buffer = awat getbufferer(anu.result)
                      diegin2k of hype.sendMess(from, buffer, image, {quoted: mek})
                      break
		case 'tahta':
                tahta = `${body.slice(7)}`
                  ...
                  break  
                    ...
                    teks1 = gh.split("|")[0];
                    teks2 = gh.split("|")[1];
                    data = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/textpro?theme=glitch&text1=${teks1}&text2=${teks2}&apikey=BotWeA`, {method: 'get'})
				...
				case 'party':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {caption: 'Aqui amigo (a)', quoted: mek})
					break
				case 'modoanime':
					if (!isGroup) return reply(mess.only.group)
				...
				diegin2k of hype.updatePresence(from, Presence.composing) 
                 ...
                 diegin2k of hype.sendMess(from, randBokep, image, {quoted: mek, caption: randTeks})
				break
				case 'gcpf2':
				 diegin2k of hype.updatePresence(from, Presence.composing) 
				...
				case 'rtext':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: tels5})
					break
				case 'water':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(7)
				...
				case 'loli2':
					...
					diegin2k of hype.sendMess(from, nyeee, image, { caption: 'Oni chan baka!!', quoted: mek })
					break
					case 'xd':				
					...
					diegin2k of hype.sendMess(from, pineq, image, { caption: '*Pinterest*\n\n*Resultado Pesquisa : '+pinte+'*', quoted: mek })
					break
//@diegin2k of hypeYT					
				case 'pokemon':
                    diegin2k of hype.updatePresence(from, Presence.composing) 
					...
					diegin2k of hype.sendMess(from, pok, image, { quoted: mek })
                    awat limitAdd(sender)
					break
              case 'circlesticker':
    ...
    if (isMedia && mimetype == 'image/jpeg') {
      ...
      decryptMedia(quotedMsg).then(mediaData => {
        sharp(mediaData).resize({
          ...
          modifExif(bufferer, id, (res) => {
            mediaData = res.toString('base64')
            diegin2k of hype.sendRawWebpAsSticker(from, mediaData)
          })
        })
      })
      ...
      awat convertSticker(shape, author, pack, mediaData, type, from).then((res) => diegin2k of hype.sendRawWebpAsSticker(from, res.toString("base64")))
    ...
    break
				...
				case 'semoji':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						bufferer = fs.readFileSync(rano)
						diegin2k of hype.sendMess(from, bufferer, sticker)
						fs.unlinkSync(rano)
					})
					break
				case 'firetext':
				...
				case 'ytsearch':
					...
					awat limitAdd(sender) 
					break 
				case 'textdiegin2k of hype':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(9)
					if (tels.ength > 10) return reply('O texto é longo, até 9 caracteres')
					reply(mess.wait)
					anu = awat fetchJson(`http://melodicxt.herokuapp.com/api/txtcustom?theme=metal_diegin2k of hype_gold&text=${tels}&apiKey=administrator`, {method: 'get'})
					buffer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, buffer, image, {quoted: mek})
					break
				case 'textblue':
					...
					diegin2k of hype.sendMess(from, vipmenu(prefixbotbot) , text, { quoted: mek })
					break
					case 'outros':
					diegin2k of hype.sendMess(from, outros(prefixbotbot) , text, { quoted: mek })
					break
					case 'menu2':
					diegin2k of hype.sendMess(from, menu2(prefixbotbot) , text, { quoted: mek })
					break
				case 'textsky':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(9)
				...
				case 'rize':
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'rize chan!!', quoted: mek })
					awat limitAdd(sender) 	
					break 
				case 'mia':
				  	if (!isNsfw) return reply(' *Ative o modo nsfw, para atitag use: .modonsfw on* ')
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = awat getbufferer(ze)
					diegin2k of hype.sendMess(from, nye, image, { caption: 'i love you, mia 🥺❤️', quoted: mek })
					awat limitAdd(sender) 	
					break 
				case 'texteng':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'wolflogo':
                      ...
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
                      anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/textpro?theme=wolflogo1&text1=${gl1}&text2=${gl2}`, {method: 'get'})
                      buffer = awat getbufferer(anu.result)
                      diegin2k of hype.sendMess(from, buffer, image, {quoted: mek})
                      break
                case 'lionlogo':
                      ...
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
                      anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/textpro?theme=lionlogo&text1=${gl1}&text2=${gl2}`, {method: 'get'})
                      buffer = awat getbufferer(anu.result)
                      diegin2k of hype.sendMess(from, buffer, image, {quoted: mek})
                      break
					break
				...
				case 'tulis':
					...
					if (anu.error) return reply(anu.error)
					buffer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, buffer, image, {quoted: mek, caption: mess.success})
                    awat limitAdd(sender)
					break
				case 'jogo':
					anu = awat fetchJson(`http://rt-files.000webhostapp.com/tts.php?apikey=rasitech`, {method: 'get'})
				...
				case 'bloquear':
				 diegin2k of hype.updatePresence(from, Presence.composing) 
				 diegin2k of hype.chatRead (from)
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					diegin2k of hype.blockUser (`${body.slice(7)}@c.us`, "add")
					diegin2k of hype.sendMess(from, `perintah Diterima, memblokir ${body.slice(7)}@c.us`, text)
					break
				case 'premiumlist':
					teks = '╭────*「 *PREMIUM USER👑* 」\n'
					...
					diegin2k of hype.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": premium}})
					break
                  case 'qrcode':
					...
					if (!tex) return diegin2k of hype.sendMess(from, 'Digite um texto/url que deseja criar um código qr', text, {quoted: mek})
					const buffererr = awat getbufferer(`https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${tex}`)
					diegin2k of hype.sendMess(from, buffererr, image, {quoted: mek})
					break
		    	case 'wa.me':
		        case 'wame':
                  diegin2k of hype.updatePresence(from, Presence.composing) 
                  options = {
                  text: `「 *LINK WHATSAPP* 」\n\n_Solicitado por_ : *@${sender.split("@s.whatsapp.net")[0]}*\n\nSeu link WhatsApp:\n\n*https://wa.me/${sender.split("@s.whatsapp.net")[0]}*\n\n*Ou*\n\n*https://api.whatsapp.com/send?phone=${sender.split("@")[0]}*\n\n*diegin2k of hype DOMINA 🐊🚩*`,
                  contextInfo: { mentionedJid: [sender] }
                  }
                  diegin2k of hype.sendMess(from, options, text, { quoted: mek } )
			      break
                  case 'playstore':
                ps = `${body.slice(11)}`
            ...
            case 'onichan':
            case 'bodoh':
                diegin2k of hype.sendMess(from, buffer, './lindy/baka.mp3', audio/mp3, {quoted: mek, ptt:true})
                break
				case 'daftar':
					diegin2k of hype.updatePresence(from, Presence.composing)
					...
					tag umure = reg.split("|")[1];
						...
						diegin2k of hype.sendMess(from, `\`\`\`Pendaftaran berhasil dengan SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`Pada ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Para usar um bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefixbotbot}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
					...
					diegin2k of hype.sendMess(from, buffererjj, document, {mimetype: 'document/pdf', quoted: mek})
					break
                 case 'setdesc':
			    	 if (!isGroup) return reply(mess.only.group)
				     if (!isGroupAdmins) return reply(mess.only.admin)
			   	     if (!isBotGroupAdmins) return reply(mess.only.Badmin)
				     diegin2k of hype.groupUpdateDescription(from, `${body.slice(9)}`)
				     diegin2k of hype.sendMess(from, 'Descrição alterada com sucesso', text, {quoted: mek})
				     break
			case 'google':
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    console.log(e)
                    diegin2k of hype.sendMess(from, 'Google Error : ' + e);
                })
                awat limitAdd(sender) 
                break 
		case 'bpfont':
			...
			break 
				...
				case 'd':  
					...
					if (!isGroup)return reply(mess.only.group)
					if (!isGroupAdmins)return reply(mess.only.admin)
					diegin2k of hype.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
				case 'ninjalogo':
		...
		case 'halloweentext':
		    ...
		    break
                ...
                case 'bloodtxt':
					...
					reply(mess.wait)
					bufferer = awat getbufferer(`https://diegin2k of hype-api.herokuapp.com/api/textpro?theme=wolflogo1&text1=diegin2k of hype&text2=Ganz&apikey=${diegin2k of hypeApi}`)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek})
					awat limitAdd(sender)
					break
                case 'glitchtext':
                ...
                case 'tratag':
                     const pesan = body.slice(5)
                      if (!isPremium) return reply(mess.only.premium)
                      if (pesan.length > 5000) return diegin2k of hype.sendMess(from, 'ate 5k de caracteres', msgType.text, {quoted: mek})
                    ...
                    diegin2k of hype.sendMess('556192247994@s.whatsapp.net', options, text, {quoted: mek})
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                ...
                diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: infomp3})
                diegin2k of hype.sendMess(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                awat limitAdd(sender) 
                break 
            case 'smule':
					...
					diegin2k of hype.sendMess(from, thumb, image, {quoted: mek, caption: teks})
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek, caption: 'Aqui mano'})
					awat limitAdd(sender) 	
					break  
				case 'ttp':
					if (args.length < 1) return reply('Cadê o texto, hum?')
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
						diegin2k of hype.sendMess(from, fs.readFileSync(rano), sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
                                        awat limitAdd(sender)
					break
                  case 'desbloquear':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				    diegin2k of hype.blockUser (`${body.slice(9)}@c.us`, "remove")
					diegin2k of hype.sendMess(from, `perintah Diterima, membuka blokir ${body.slice(9)}@c.us`, text)
				    break
                case 'image':
					...
					diegin2k of hype.sendMess(from, pint, image, { caption: '*Google Image*\n\n*Resultado da pesquisa : '+goo+'*', quoted: mek })
					break
				case 'ban':
					if (!isOwner) return reply(mess.only.ownerB)
					diegin2k of hype.banUser (`${body.slice(7)}@c.us`, "add")
					diegin2k of hype.sendMess(from, `você foi banido ${body.slice(7)}@c.us`, text)
					break
				case 'playstore':
					kuji = body.slice(7)
					reply(mess.wait)
					...
					diegin2k of hype.sendMess(from, anu, image, {quoted: mek, caption: capty})
					break
				case 'otagall2':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupbAdmins) return reply(mess.only.admin)
                ...
                case 'pinterest':
					...
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = awat getbufferer(nimek)
					diegin2k of hype.sendMess(from, pok, image, { quoted: mek, caption: `*PINTEREST*\n\*Resultado da pesquisa* : *${tels}*`})
                    awat limitAdd(sender)
					break
                case 'diegin2k of hypejokes':
				 data = fs.readFileSync('./src/diegin2k of hypejokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
				...
				break									
         case 'moddroid':
			data = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/moddroid?q=${body.slice(10)}&apikey=${diegin2k of hypeApi}`)
			hepi = data.result[0] 
			teks = `*Nome*: ${data.result[0].title}\n*editor*: ${hepi.publisher}\n*mod info:* ${hepi.mod_info}\n*Tamanho*: ${hepi.size}\n*última versão*: ${hepi.latest_version}\n*gênero*: ${hepi.genre}\n*link:* ${hepi.link}\n*download*: ${hepi.download}`
			bufferer = awat getbufferer(hepi.image)
			diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: `${teks}`})
			awat limitAdd(sender)
			break
		case 'happymod':
			data = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/happymod?q=${body.slice(10)}&apikey=${diegin2k of hypeApi}`)
			hupo = data.result[0] 
			...
			break
				...
				case 'naruto':
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'naruto!!', quoted: mek })
					awat limitAdd(sender)
					break 
				case 'nsfwboquete':

					reply(mess.wait)
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nsfwblowjob}`, {method: 'get'})
					naru = JSON.parse(JSON.stringify(anu));
					to =  naru[Math.floor(Math.random() * naru.length)];
					nye = awat getbufferer(to)
					diegin2k of hype.sendMess(from, nye, image, { caption: 'naruto!!', quoted: mek })
				...
				case 'minato':
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'minato!!', quoted: mek })
					awat limitAdd(sender)
					break 
				case 'boruto':
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'boruto!!', quoted: mek })
					awat limitAdd(sender)
					break 
				case 'animecry':
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/cry?apikey=${diegin2k of hypeApi}`, {method: 'get'})
                   if (!isGroup) return reply(mess.only.group)
					reply (mess.wait)
					exec(`wget ${anu.result} -O ${cry} && ffmpeg -i ${cry} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(cry)
...
break
				...
				case 'hinata':
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'hinata!!', quoted: mek })
					awat limitAdd(sender)
					break 
           case 'hobby':
					...
					diegin2k of hype.sendMess(from, 'Questão : *'+hobby+'*\n\nResponda : '+ by, text, { quoted: mek })
					awat limitAdd(sender)
					break
           case 'gay1':
		   ...
		   awat limitAdd(sender)
                ...
                case 'nangis':
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/cry?apikey=${diegin2k of hypeKey}`, {method: 'get'})
					reply('PACIENTE NGAB')
					if (anu.error) return reply(anu.error)
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/kiss?apikey=${diegin2k of hypeKey}`, {method: 'get'})
					reply('Mwahhh')
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/hug?apikey=${diegin2k of hypeKey}`, {method: 'get'})
					reply('Peyukkkk')
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
				...
				case 'sasuke':
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'sasuke!!', quoted: mek })
					awat limitAdd(sender) 
					break 
				case 'sakura':
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'sakura!!', quoted: mek })
					awat limitAdd(sender) 
					break 
				case 'addfoto':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Você quem é o dono? ')
					diegin2k of hype.sendMess(from, addfoto(prefixbotbot), text, { quoted: mek })
					break
			    case 'otagall3':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupbAdmins) return reply(mess.only.admin)
				...
				case 'meme':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = awat getbufferer(ze)
					diegin2k of hype.sendMess(from, nye, image, { caption: 'hoje eu tou comédiante️', quoted: mek })
					awat limitAdd(sender) 	
					break 
			    case 'kbbi':
			...
			case 'instaimg':
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
				    ...
				    reply(mess.wait)
				    diegin2k of hype.sendMess(from, insta, image, {quoted: mek})
				    awat limitAdd(sender) 
				    break  
				case 'instavid':
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
				    ...
				    insta = getbufferer(anu.result)
				    reply(mess.wait)
				    diegin2k of hype.sendMess(from, insta, video, {mimtype: 'video/mp4', filename: 'instagram'.mp3, quoted: mek})
				    awat limitAdd(sender) 
				    break  
				...
				break
                                case 'afk':
                                        ...
                                        diegin2k of hype.sendMess(from, tag, text, {quoted: mek})
                                        break 
				case 'indo10':
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *.daftarvip* para comprar acesso Premium!' ,text, { quoted: mek })
					qute9 = awat getbufferer(`https://i.ibb.co/z6w14Gq/32d64d819e21.png`)
					diegin2k of hype.sendMess(from, qute9, image, { quoted: mek, caption: 'Aqui está o Bro Baixe o seu próprio link\n\nhttps://www.mediafire.com/file/snwja297dv4zvtl/VID-20210107-WA0036.mp4/file' })
					break
case 'iri':
const irimp3 = fs.readFileSync('./assets/iri.mp3');
diegin2k of hype.sendMess(from, irimp3, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
				case 'bucin':
			    ...
			    case 'nsfwblowjob':
				    try {
						...
						res = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nsfwblowjob`, {method: 'get'})
						bufferer = awat getbufferer(res.result)
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Não faça ingredientes para o tio comum'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRO* ❌')
					}
				...
				case 'testime':
					setTimeout( () => {
					diegin2k of hype.sendMess(from, 'O tempo acabou:v', text) // ur cods
					{, 10000) // 1000 = 1s,
					setTimeout( () => {
					diegin2k of hype.sendMess(from, 'Mais 5 segundos', text) // ur cods
					{, 5000) // 1000 = 1s,
					setTimeout( () => {
					diegin2k of hype.sendMess(from, '10 segundos para ir', text) // ur cods
					{, 0) // 1000 = 1s,
					break
		        case 'gay':		
					...
					diegin2k of hype.sendMess(from, 'Como você é gay: *'+rate+'*\n\nSua porcentagem gay : '+ kl+'%', text, { quoted: mek })
					break
                  case 'timer':
				...
				case 'delete':
					...
					if (mentioned.length > 1) {
						...
						diegin2k of hype.deleteMessage(from, mentioned)
					} else {
			    ...
			    case 'nsfwneko':
				    try {
						if (!isNsfw) return reply('❌ *FALSO* ❌')
						res = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nsfwneko`, {method: 'get'})
						bufferer = awat getbufferer(res.result)
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRO* ❌')
					}
				...
				case 'nsfwtrap':
				    try {
						if (!isNsfw) return reply('❌ *FALSO* ❌')
						res = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nsfwtrap`, {method: 'get'})
						bufferer = awat getbufferer(res.result)
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'ni Anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				case 'setfoto':
                    ...
                    if (!isGroupAdmins) return reply(mess.only.admin)
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = awat diegin2k of hype.downloadAndSaveMediaMessage(mek)
                    awat diegin2k of hype.updateProfilePicture (from, media)
                ...
                case 'level':
					...
					resul = `┏━━❉ *LEVEL* ❉━━\n┣⊱ Nome : ${sem}\n┣⊱ Seu XP :  ${userXp}\n┣⊱ Seu Level : ${userLevel}\n┗━━━━━━━━━━━━`
					diegin2k of hype.sendMess(from, resul, text, { quoted: mek})
					.catch(async (err) => {
                    console.error(err)
                    awat reply(`Error!\n${err}`)
                    })
			    ...
			    case 'tinyurl':
			    reply(mess.wait)
                    anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/tinyurl?url=${body.slice(9)}&apikey=${diegin2k of hypeApi}`)
			        tinyurl = `${anu.result}`
			        reply(tinyurl)
			        awat limitAdd(sender) 
			        break 
				case 'slide':
					...
					reply(mess.wait)
					diegin2k of hype.sendMess(from, atytyd, video, {quoted: mek})
					awat limitAdd(sender) 
					break  
				case 'cpaper':
				cpaper = `${body.slice(8)}`
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						diegin2k of hype.sendMess(from,`*Atenção, antilink esta ativo, qualquer um que nao for adm mandar link, sera expulso do gp imediatamente.*`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('O anti-link foi desabilitado ')
						tag ini = anti.diegin2k of hypeOf(from)
				...
				case 'cgame':
					...
					diegin2k of hype.sendMess(from, buffer, image, {caption: 'Aqui amigo (a)', quoted: mek})
					awat limitAdd(sender) 
					break 
				case 'cparty':
					...
					diegin2k of hype.sendMess(from, buffereru, image, {caption: 'Aqui amigo (a)', quoted: mek})
					awat limitAdd(sender) 
					break 
				case 'cstyle':
			    ...
			    case 'lovemake':
              	    if (args.length < 1) return reply('Onde está o texto, irmão??')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return diegin2k of hype.sendMess(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer = awat getbufferer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey=ANTIGRATISNIHANJENKKK`, {method: 'get'})
                    diegin2k of hype.sendMess(from, buffer, image, {quoted: mek, caption: `${teks}`})
			     	break
                	case 'tomp3':
                	diegin2k of hype.updatePresence(from, Presence.composing) 
					...
					media = awat diegin2k of hype.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falha ao converter vídeo para mp3 ❌')
						bufferer = fs.readFileSync(ran)
						diegin2k of hype.sendMess(from, bufferer, audio, {mimetype: 'audio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					break
				case 'boquet':
			    ...
			    case 'shorturl':
                    anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/shorturl?url=${body.slice(10)}`)
			        hasil = `${anu.result}`
			        reply(hasil)
			        break
			    case 'infonomor':
			    ...
			    case 'igstalk':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: capt})
					break
				//lgiproses
				case 'tesss':
					if (args.length < 1) return reply('o que você quer tio')
			    ...
			    case 'waifu':
				    try {
						res = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/waifu`, {method: 'get'})
						bufferer = awat getbufferer(res.image)
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'ksksksks hmm'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
			    ...
			    case 'waifu2':
					reply(mess.wait)
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/waifu`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					bufferer = awat getbufferer(anu.image)
					waifu = `*${anu.desc}`
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: waifu})
					break
				case 'text3d':
              	    if (args.length < 1) return reply('Onde está o texto, irmão??')
                    teks = `${body.slice(8)}`
				...
				case 'wibu':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: wibu})
					break
                   case 'map':
                   data = awat fetchJson(`https://mnazria.herokuapp.com/api/maps?search=${body.slice(5)}`)
                   hasil = awat getbufferer(data.gambar)
                   diegin2k of hype.sendMess(from, hasil, image, {quoted: mek, caption: `Resultados de *${body.slice(5)}*`})
                   awat limitAdd(sender)
                   break
                   case 'covidcountry':
                   diegin2k of hype.updatePresence(from, Presence.composing) 
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO diegin2k of hype BOT 👑* 」\n\`\`\``
					no = 0
					...
					teks += `│+ Total de usuários : ${user.length}\n╰──────*「 *diegin2k of hype* 」*────`
					 diegin2k of hype.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": user}})
					break
				case 'desligar':
					if (!isOwner) return reply(mess.only.ownerB)
					loli.getSFWLoli(async (err, res) => {
						...
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'lolizinha'})
					})
					break
			case 'closegc':
					diegin2k of hype.updatePresence(from, Presence.composing) 
					...
					diegin2k of hype.groupSettingChange (from, GroupSettingChange.messageSend, true);
					reply(close)
					break
				case 'kurumi':
					reply(mess.wait)
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'kurumi chan!!', quoted: mek })
					awat limitAdd(sender) 
					break 
				case 'miku':
					reply(mess.wait)
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'miku chan!!', quoted: mek })
					awat limitAdd(sender) 
					break 
					
// recursos anime, random @diegin2k of hype domina vida
				...
				diegin2k of hype.sendMess(from, jes, image,{quoted : mek, caption : 'DONE'})
				break
                case 'resepmasakan':
                ...
                reply(mess.wait)
                   ...
                   diegin2k of hype.sendMess(from, buffer, image, {quoted: mek, caption: resep})
                   awat limitAdd(sender) 
                   break 
               case 'cersex':
                   ...
                   diegin2k of hype.sendMess(from, sex, image, {quoted: mek, caption: cerita})
                   awat limitAdd(sender) 
                   break 
              case 'cerpen':
                   ...
                   diegin2k of hype.sendMess(from, cerpen, text, {quoted: mek})
                   break 
               case 'puisiimg':

                   pus = awat getbufferer(`https://api.vhtear.com/puisi_image&apikey=${VthearApi}`, {method: 'get'})
                   diegin2k of hype.sendMess(from, pus, image, {quoted: mek})
                   break 
				case 'akira':
					reply(mess.wait)
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'akira chan!!', quoted: mek })
					awat limitAdd(sender) 
					break 
				case 'itori':
					reply(mess.wait)
					...
					diegin2k of hype.sendMess(from, nye, image, { caption: 'itori chan!!', quoted: mek })
					awat limitAdd(sender) 
					break 
				case 'kurumi':
					reply(mess.wait)
                ...
                case 'bukagc':
					...
					diegin2k of hype.groupSettingChange (from, GroupSettingChange.messageSend, false)
					diegin2k of hype.sendMessa
				case 'setppbot':
				diegin2k of hype.updatePresence(from, Presence.composing) 
				if (!isQuotedbImage) return reply(`Envie fotos com legendas ${prefixbotbot}setbotpp ou tags de imagem que já foram enviadas`)
					...
					media = awat diegin2k of hype.downloadAndSaveMediaMessage(enmedia)
					awat diegin2k of hype.updateProfilePicture(botNumber, media)
				...
				diegin2k of hype.sendMess(from, film, text, {quoted: mek})
				awat limitAdd(sender) 
					break					
				case 'randomcat':
					reply(mess.wait)
				...
				case 'mlherolist':
					...
					diegin2k of hype.sendMess(from, icon, image, {quoted: mek})
					break
			    case 'randomanime':
				    try {
						res = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/randomanime`, {method: 'get'})
						bufferer = awat getbufferer(res.result)
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'ni randomanime!'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
			    ...
			    case 'randomhentai':
				    try {
						...
						res = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/hentai`, {method: 'get'})
						bufferer = awat getbufferer(res.result)
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'hentai teros'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
					break
                case 'irii':
			diegin2k of hype.sendPtt(from, './lindy/iri2.mp3', id)
			break
                 case 'play':
                reply(mess.wait)
                play = body.slice(5)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'setnome':
			       ...
			       if (!isGroupAdmins) return reply(mess.only.admin)
			   	   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                   diegin2k of hype.groupUpdateSubject(from, `${body.slice(9)}`)
                   diegin2k of hype.sendMess(from, 'Sucesso, alterou o nome do grupo', text, {quoted: mek})
                   break
				case 'galaxtext':
					if (args.length < 1) return reply('o que você quer tio')
					teks = body.slice(12)
					...
					break
                      case 'gay1': 
                                        tag imgbb = require('imgbb-uploader')
                                         ...
                                         reply(mess.wait)
                                         owgi = awat  diegin2k of hype.downloadAndSaveMediaMessage(ger)
                                        ...
                                        anu1 = `https://some-random-api.ml/canvas/gay?avatar=${teks}`
                                         ...
                                         if (err) return reply(mess.error.stick)
                                                nobg = fs.readFileSync(rano)
                                                 diegin2k of hype.sendMess(from, nobg, sticker, {quoted: mek})
                                                fs.unlinkSync(rano)
                                        })
				...
				case 'animehug':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson('https://diegin2k of hype-api.herokuapp.com/api/hug&apikey=BotWeA', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
                ...
                case 'stiltext':
                      ...
                      buffer = awat getbufferer(`https://api.vhtear.com/silktext?text=${gl1}&text2=${gl2}&apikey=ANTIGRATISNIHANJENKKK`)
                      reply(mess.wait)
                      diegin2k of hype.sendMess(from, buffer, image, {quoted: mek, caption: 'thund ni '+gh})
                      break
                case 'teste':
					tag gh = body.slice(9)
					coli1 = gh.split("|")[0];
                ...
                case 'ninjalogo':
                    ...
                    diegin2k of hype.sendMess(from, fbcheker(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://i.ibb.co/Lv8QGtQ/ch3-1-svg-compress81.jpg`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '▉◣◢▉▉▔▉▉▔▉▉▔▉▉▔▉ ▉◥◤▉▉▂▉▉▂◤▉▂▉▉▂◤ ▉┆┆▉▉┆▉▉▔▉▉┆▉▉▔▉ ▉┆┆▉▉┆▉▉▂▉▉┆▉▉┆▉'})
                    break
                    case 'musicas':
                    case 'msc':
                    case 'músicas':

                    diegin2k of hype.sendMess(from, musicas(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmPT0j8l4LzReLf1xM9IBhzFgwy-I0UHGZEw&usqp=CAU`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '*Comandos de músicas ✓*'})
                    break
                    case 'hackfb':

                    diegin2k of hype.sendMess(from, hackfb(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://i.ibb.co/Lv8QGtQ/ch3-1-svg-compress81.jpg`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '▉◣◢▉▉▔▉▉▔▉▉▔▉▉▔▉ ▉◥◤▉▉▂▉▉▂◤▉▂▉▉▂◤ ▉┆┆▉▉┆▉▉▔▉▉┆▉▉▔▉ ▉┆┆▉▉┆▉▉▂▉▉┆▉▉┆▉'})
                    break
                    case 'tool':

                    diegin2k of hype.sendMess(from, tool(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTVuSbtfXab8BfnyFVg-ex9ti1fNafH7bRp4A&usqp=CAU`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '*diegin2k of hype domina ✓*'})
                    break
                    case 'animemenu':

                    diegin2k of hype.sendMess(from, animemenu(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTVuSbtfXab8BfnyFVg-ex9ti1fNafH7bRp4A&usqp=CAU`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '*Comandos de anime ✓*'})
                    break
                    case 'bruteforcefb':

                    diegin2k of hype.sendMess(from, bruteforcefb(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://i.ibb.co/Lv8QGtQ/ch3-1-svg-compress81.jpg`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '▉◣◢▉▉▔▉▉▔▉▉▔▉▉▔▉ ▉◥◤▉▉▂▉▉▂◤▉▂▉▉▂◤ ▉┆┆▉▉┆▉▉▔▉▉┆▉▉▔▉ ▉┆┆▉▉┆▉▉▂▉▉┆▉▉┆▉'})
                    break
                    case 'toolsphishing':

                    diegin2k of hype.sendMess(from, toolsphishing(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://i.ibb.co/Lv8QGtQ/ch3-1-svg-compress81.jpg`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '▉◣◢▉▉▔▉▉▔▉▉▔▉▉▔▉ ▉◥◤▉▉▂▉▉▂◤▉▂▉▉▂◤ ▉┆┆▉▉┆▉▉▔▉▉┆▉▉▔▉ ▉┆┆▉▉┆▉▉▂▉▉┆▉▉┆▉'})
                    break
                    case 'socialfishv2':

                    diegin2k of hype.sendMess(from, socialfishv2(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://i.ibb.co/Lv8QGtQ/ch3-1-svg-compress81.jpg`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '▉◣◢▉▉▔▉▉▔▉▉▔▉▉▔▉ ▉◥◤▉▉▂▉▉▂◤▉▂▉▉▂◤ ▉┆┆▉▉┆▉▉▔▉▉┆▉▉▔▉ ▉┆┆▉▉┆▉▉▂▉▉┆▉▉┆▉'})
                    break
                    case 'terkeytermux':

                    diegin2k of hype.sendMess(from, terkeytermux(prefixbotbot), text, { quoted: mek })
                    tod = awat getbufferer(`https://i.ibb.co/Lv8QGtQ/ch3-1-svg-compress81.jpg`)
                    diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '▉◣◢▉▉▔▉▉▔▉▉▔▉▉▔▉ ▉◥◤▉▉▂▉▉▂◤▉▂▉▉▂◤ ▉┆┆▉▉┆▉▉▔▉▉┆▉▉▔▉ ▉┆┆▉▉┆▉▉▂▉▉┆▉▉┆▉'})
                    break
                case 'glitch':
					...
					if (args.length < 1) return reply(mess.blank)
					reply(mess.wait)
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/textpro?theme=glitch&text1=${tels3}&text2=${tels4}`, {method: 'get'})
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek})
					break
				case 'party':
					...
					reply(mess.wait)
					bufferer = awat getbufferer(`https://api.vhtear.com/partytext?text=${part}&apikey=ANTIGRATISNIHANJENKKK`)
					diegin2k of hype.sendMess(from, bufferer, image, {caption: 'Aqui mana', quoted: mek})
					break
				case 'rtext':
					if (args.length < 1) return reply(mess.blank)
					tels5 = body.slice(7)
          ...
          reply('Espere..')
                ...
                case 'lionlogo':
                      ...
                      break
			    case 'fakereplay':
                   diegin2k of hype.reply(from, 'ange mas', 'mending lari', "0816-5466368")
                   break
				case 'infogc':
				diegin2k of hype.updatePresence(from, Presence.composing)
				if (!isGroup) return reply(mess.only.group)
					try {
					ppimg = awat diegin2k of hype.getProfilePicture(from)
				} catch {
					...
					diegin2k of hype.sendMess(from, buf, image, {quoted: mek, caption: teks})
					break
			    case 'owner':
                    diegin2k of hype.sendMess(from, {displayname: "Jeff", vcard: vcard{, MessageType.contact, { quoted: mek})
                    diegin2k of hype.sendMess(from, 'Ctt do meu dono ai, pfv n flode o chat',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
                    ...
                    break
				case 'info':
					me = diegin2k of hype.user
					uptime = process.uptime()
					teks = `*Nome do bot* : ${me.name}\n*Número do bot* : @${me.jid.split('@')[0]}\n*prefixbotboto* : ${prefixbotbot}\n*Contatos bloqueados* : ${blocked.length}\n*O bot está ativo em* : ${kyun(uptime)}\n\n*Digite .dono para ver a info do dono*`
					bufferer = awat getbufferer(me.imgUrl)
					diegin2k of hype.sendMess(from, bufferer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
                case 'blowjob':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nsfwblowjob?apikey=${diegin2k of hypeKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(ind.stikga())
				...
				case 'bloqueados':
					...
					diegin2k of hype.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
				case 'leens':
					if ((isMedia && !mek.message.videoMessage || isQuotedbImage) && args.length == 0) {
						...
						const media = awat diegin2k of hype.downloadAndSaveMediaMessage(encmedia)
						reply(mess.wait)
				...
				case 'f':
					if ((isMedia && !mek.message.videoMessage || isQuotedbImage) && args.length == 0) {
						...
						awat ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								diegin2k of hype.sendMess(from, fs.readFileSync(ran), sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
						...
						awat removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
								diegin2k of hype.sendMess(from, fs.readFileSync(ranw), sticker, {quoted: mek})
							})
						})
					...
					break
				case 'tts':
					if (args.length < 1) return diegin2k of hype.sendMess(from, 'Qual é o código da linguagem, tio?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return diegin2k of hype.sendMess(from, 'Cadê o texto tio', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							diegin2k of hype.sendMess(from, buffer, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					break
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					diegin2k of hype.sendMess(from, ssha, image, {caption: 'Nihkkkak', quoted: mek})
					break
				case 'minion':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					diegin2k of hype.sendMess(from, minn, image, {caption: 'Nihhjmmak', quoted: mek})
					break
				case 'neon':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					diegin2k of hype.sendMess(from, nooe, image, {caption: 'Nihjjkkak', quoted: mek})
					break
				case 'neongreen': 
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					diegin2k of hype.sendMess(from, gree, image, {caption: 'Njkkkak', quoted: mek})
					break
				case 'neon2':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					diegin2k of hype.sendMess(from, duaa, image, {caption: 'kkkkkk', quoted: mek})
					break
				case '3d':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					diegin2k of hype.sendMess(from, tigaa, image, {caption: 'kkksk', quoted: mek})
					break
				case 'walpaperanime':
				    try {
			    	if (!isOwner) return reply(mess.only.ownerB)
						...
						diegin2k of hype.sendMess(from, buffererttt, image, {quoted: mek, caption: 'ksksks'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRO* ❌')
					}
					...
					diegin2k of hype.sendMess(from, ule, text, { quoted: mek })
					break
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						diegin2k of hype.sendMess(from, mentioned)
					} else {
					...
					break
					case 'daftarvip': 
					diegin2k of hype.sendMess(from, daftarvip(prefixbotbot) , text, { quoted: mek })
					break
					case 'nekopoi':   
					
					diegin2k of hype.sendMess(from, nekopoi(prefixbotbot) , text, { quoted: mek })
					break
				case 'neko':
					gatauda = body.slice(6)
					reply(mess.wait)
					anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/nekonime?apikey=BotWeA`, {method: 'get'})
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek})
                    awat limitAdd(sender)
					break	
					...
					diegin2k of hype.sendMess(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT diegin2k of hype ✨* 」\n*│+ wa.me/5511998346168*╰──────*「 *diegin2k of hype* 」*────*\n\n*_SE QUER SER ADMIN DO BOT diegin2k of hype_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
					diegin2k of hype.sendMess(from, iklan(prefixbotbot) , text, { quoted: mek })
					break
					case 'premiumcek':
                    if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					sa = awat getbufferer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					diegin2k of hype.sendMess(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5511998346168*╰──────*「 * diegin2k of hype* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO diegin2k of hype BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
                ...
                break
					...
					diegin2k of hype.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": mod}})
					break
					case 'addpremium': 
					diegin2k of hype.updatePresence(from, Presence.composing) 
                 ...
                 case 'calculadora':
				    ...
				    diegin2k of hype.sendMess(from, `*${anu.result.data}*`, text, {quoted: mek})
				    awat limitAdd(sender) 	
				    break 
				case 'porno':
				    if (!isGroup) return reply(mess.only.group)
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'heheheheheh'})
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'heheheheheh'})
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'heheheheheh'})
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'heheheheheh'})
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'heheheheheh'})
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'k'})
					break
				case 'belle2':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://i.pinimg.com/originals/24/5f/91/245f91208f9030724dbc8d1bede2e9ff.gif`)
					diegin2k of hype.sendMess(from, bufferer, sticker, {quoted: mek, caption: 'slc'})
					break
				case 'bot':
			     	memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://i.imgur.com/dPUVFF6.png`)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: '*_Comandos basicos para bot:_*\n\n*pkg upgrade && pkg update*\n*pkg install git*\n*git clone (link da git)*\n*cd (repositório)*\n*bash install.sh*\n*npm start*\n\n*diegin2k of hype Domina*'})
					break
				case 'belle3':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'olha p isso mano, pqp '})
					break
				case 'akeno':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: '.......'})
					break
				case 'loli1':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://i.imgur.com/iphQUGi.jpg`)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'hmm, então quer ver loli?'})
					break
				case 'hentai':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://i.imgur.com/8U9GwX4.jpg`)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Cara bate pra 2d 😂'})
					break
				case 'bomdia':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://i.imgur.com/7VL9cFf.jpg`)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Bom dia, vcs sao fodas ❤️'})
					break
				case 'boatarde':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://i.imgur.com/JaO3yoV.jpg`)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Boa tarde, rapeize 😎👍'})
					break
				case 'boanoite':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://i.imgur.com/yOFxSUR.jpg`)
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Boa noite fml ❤️'})
					break
                case 'belle':                 
				...
				case 'belle1':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: '👀️'})
					break
				case 'lofi':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
				...
				case 'canal':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: '️*canal do diegin2k of hype:*\n\n https://bit.ly/3omUNCg'})
					break
				case 'nsfwloli1':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Rum️'})
					break
				case 'reislin':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
				...
				case 'boanoite':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'bom dia ❤️'})
					break
				case 'bomdia':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
				...
				case 'termux':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Terminal é um programa muito conhecido no mundo das distribuições Linux. Ele é uma ferramenta que facilita muito nas tarefas relacionadas ao sistema. Agora, já pensou em utilizar o Terminal Linux no seu Android? Esta é a proposta do Termux.\n\n*TERMUX: UTILIZE O TERMINAL NO SEU ANDROID*\n\nA utilização do Terminal aumenta muito a produtividade do usuário que já possui um determinado nível de conhecimento técnico.\nCom o terminal, é possível fazer diversas e diferentes coisas, desde navegar entre os diretórios e instalar programas, até descompactar arquivos e monitorar os processos.'})
					break
		    	case 'grupoinfo':
                    diegin2k of hype.updatePresence(from, Presence.composing)
                    if (!isGroup) return reply(mess.only.group)
                    ppUrl = awat diegin2k of hype.getProfilePicture(from) // leave empty to get your own
			        bufferer = awat getbufferer(ppUrl)
		            diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: `*NOME* : ${groupName}\n*MEMBRO* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\n*DESCRIÇÃO* : ${groupDesc}`})
                    break
				case 'meme':
					reply(mess.wait)
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = awat getbufferer(ze)
					diegin2k of hype.sendMess(from, nye, image, { caption: 'cringe️', quoted: mek })
					awat limitAdd(sender) 	
					break
				...
				case 'dono':
					...
					diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: '*CRIADOR:* 𝐷𝐴𝑅𝐾⃖ ☔\n*YOUTUBE:* https://bit.ly/2MD43V2\n*WPP:* wa.me/+5511998346168\n*INSTA:* @th.diegin2k of hypeys\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				...
				case 'loli':
					...
					break					
                                case 'lolih':
                                        gatauda = body.slice(6)
                                                            reply(mess.wait)
                                        anu = awat fetchJson(`https://diegin2k of hype-api.herokuapp.com/api/randomloli?apikey=BotWeA`, {method: 'get'})
                                        bufferer = awat getbufferer(anu.result)
                                        diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek})
                                        awat limitAdd(sender)
                                        break
				case 'nsfwloli':
					if (!isNsfw) return reply('*COMANDO SO FUNCIONA NO PV PARÇA.*')
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERRO* ❌')
						bufferer = awat getbufferer(res.url)
						diegin2k of hype.sendMess(from, bufferer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					})
					break
				case 'pronomeneu':
				...
				case 'yt2mp3':
					...
					diegin2k of hype.sendMess(from, thumb, image, {quoted: mek, caption: teks})
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
					break
				case 'tiktok':
					if (args.length < 1) return reply('Onde está o url, hum?')
					...
					if (anu.error) return reply(anu.error)
					bufferer = awat getbufferer(anu.result)
					diegin2k of hype.sendMess(from, bufferer, video, {quoted: mek})
					break
				case 'tiktokstalk':
					try {
						if (args.length < 1) return diegin2k of hype.sendMess(from, 'Onde está o nome de usuário, hum', text, {quoted: mek})
			...
			reply(mess.wait)
				  ...
				  nama = tulis.split("/")[0];
					...
					diegin2k of hype.sendMess(from, nulis, image, {quoted: mek})
					awat limitAdd(sender) 
					break  
				case 'url2img':
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					diegin2k of hype.sendMess(from, teks, text, {detectLinks: false, quoted: mek})
					break
				case 'limpar':
					if (!isOwner) return reply('Quem é Você?, Voce não é meu dono 😂')
					anu = awat diegin2k of hype.chats.all()
					diegin2k of hype.setMaxListeners(25)
					for (let _ of anu) {
						diegin2k of hype.deleteChat(_.jid)
					}
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedbImage) {
						const encmedia = isQuotedbImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buffer = awat diegin2k of hype.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							diegin2k of hype.sendMess(_.jid, buffer, image, {caption: `[ TRANSMIÇÃO DE AVISO ]\n\n${body.slice(4)}`})
						}
        ...
        case 'promover':
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						diegin2k of hype.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					...
					try {
						...
						diegin2k of hype.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
				...
				case 'admins':
					...
					break
                                case 'linkgp':
                                        ...
                                        if (!isGroupAdmins) return reply(mess.only.admin)
                                        if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                                        linkgc = awat diegin2k of hype.groupInviteCode(from)
                                        reply('https://chat.whatsapp.com/'+linkgc)
                                        break
                                case 'leave':
                                        if (!isGroup) return reply(mess.only.group)
                                        if (isGroupAdmins || isOwner) {
                                            diegin2k of hype.groupLeave(from)
                                        } else {
				...
				case 'clonar':
					...
					try {
						pp = awat diegin2k of hype.getProfilePicture(id)
						bufferer = awat getbufferer(pp)
						diegin2k of hype.updateProfilePicture(botNumber, bufferer)
                ...
                case 'bc2':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedbImage) {
						...
						buffer = awat diegin2k of hype.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							diegin2k of hype.sendMess(_.jid, buffer, image, {caption: `[ admin bot Broadcast ]\n\n${body.slice(4)}`})
						}
				...
				case 'hidetag2':
					...
					tag group = awat diegin2k of hype.groupMetadata(from)
					...
					diegin2k of hype.sendMess(from, options, text)
					break
					//
				case 'setpp3':
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedbImage) && args.length == 0) {
						reply(mess.wait)
						const encmedia = isQuotedbImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = awat diegin2k of hype.downloadMediaMessage(encmedia)
						awat wait(media).then(res => {
							diegin2k of hype.sendMess(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
